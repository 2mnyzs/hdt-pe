#include "ragdoll.h"
#include "ui_ragdoll.h"

Ragdoll::Ragdoll(QWidget *parent) :
...
void Ragdoll::on_radioDegree_toggled(bool checked)
{
...
hkpConstraintData* Ragdoll::getConstraint()
{
	float k = ui->radioDegree->isChecked() ? HK_REAL_DEG_TO_RAD : 1;
	auto data = new hkpRagdollConstraintData;
	...
	data->setConeMotor(ui->widgetMotorCone->getMotor());
	data->setPlaneMotor(ui->widgetMotorPlane->getMotor());
	data->setTwistMotor(ui->widgetMotorTwist->getMotor());
...
int Ragdoll::parseConstraint(const hkpConstraintData *p)
{
	auto data = dynamic_cast<const hkpRagdollConstraintData*>(p);
	...
	ui->widgetMotorCone->setMotor(data->getConeMotor());
	ui->widgetMotorPlane->setMotor(data->getPlaneMotor());
	ui->widgetMotorTwist->setMotor(data->getTwistMotor());
